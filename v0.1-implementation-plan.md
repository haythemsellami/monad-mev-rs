# monad-mev-rs v0.1 Implementation Plan

Status: Complete
Status date: 2026-05-25

> Historical implementation tracker. The SDK deferral recorded in WP-02 was
> superseded when the Linux live SDK integration landed; current dependency,
> compatibility, and release requirements live in `docs/sdk-versioning.md`,
> `docs/release-scope.md`, and `docs/release-checklist.md`.

This file is the development tracker for v0.1. The product and architecture source of truth is `v0.1-spec.md`; this file turns that spec into ordered implementation work, test coverage requirements, and release gates.

v0.1 has 6 spec milestones, but this plan breaks them into 20 concrete work packages so progress can be tracked at implementation granularity.

## 1. How To Use This Plan

- Keep `v0.1-spec.md` stable as the product contract.
...
- Any intentional scope change should update both this file and `v0.1-spec.md`.
...
## 2. v0.1 Release Gates

v0.1 implementation is complete as of 2026-05-25. Re-run these gates before tagging a v0.1 patch release:

- [ ] All 20 work packages are complete or explicitly moved to v0.2.
...
"Full coverage" for v0.1 means full behavioral coverage of framework-owned logic, not 100% line coverage of generated FFI bindings or third-party SDK code.
...
- [x] Add `README.md` with v0.1 positioning and "not production trading yet" warning.
...
- [x] Identify the exact SDK repository and tag/revision for v0.1.
...
- [x] Record SDK dependency declarations only inside `crates/monad-mev-events`; active git dependencies were deferred during WP-02 because Cargo fetched upstream submodules during ordinary lockfile resolution. They are now enabled for Linux SDK/live builds.
...
- [x] Document the schema hash API exposed by the active SDK integration.
...
At the time of WP-02, the active upstream SDK dependency build was intentionally deferred because enabling optional git dependencies caused ordinary Cargo tests to fetch upstream submodules. The later Linux live integration superseded this deferral and now pins an exact upstream revision.
...
Goal: create the three v0.1 crates with clean ownership boundaries.
...
Implementation note: v0.1 uses an SDK-compatible direct parser for compressed event-ring snapshots so normal workspace builds do not fetch the large upstream Monad SDK repository. Raw execution-event decoding remains in WP-07.
...
Goal: decode the most useful common log events for v0.1 examples and strategies.
...
Goal: make deterministic snapshot replay the central v0.1 workflow.
...
- [x] Ensure v0.1 executors cannot submit transactions.
...
Goal: provide the main v0.1 user interface.

Implementation tasks:

- [x] Choose CLI parser crate. v0.1 uses a small manual parser to avoid pulling a parser dependency before command shape stabilizes.
...
- [x] Add shell completion only if cheap; otherwise delay. Delayed to v0.2 unless a parser crate is adopted.
...
- [x] Poll descriptors. v0.1 poller is observe-only and ends cleanly until SDK-backed Linux polling is enabled.
...
- [x] Ensure no v0.1 path submits transactions.
...
Goal: make v0.1 usable and maintainable after the first implementation pass.
...
v0.1 should use these test categories:
...
v0.1 must test failure paths explicitly:
...
If one of these becomes necessary, update `v0.1-spec.md` first and explicitly move the feature from v0.2 to v0.1.
